...
assembles it. This script removes any unecessary nodes and recolors
...
Node removal and recoloring have various methods, some which are ran
...
from compose_tree import setAttributeToNode
    ...
    for node, successor_lobes in all_successor_lobes.items():
        if 1 < len(successor_lobes):
            ...
            if (n['level'] == 4 and not any(exc)) or (n['level'] == 5 and all(exc)):
                ...
                possible_new_lobes = [
                    l for l, c in sorted(successor_lobes.items(), key=lambda x: x[1], reverse=True)
                ]
                ...
                for new_lobe in possible_new_lobes:
                    difference_per_lobe_root_and_curr_node = [
                        c - successor_lobes.get(l, 0) 
                        for l, c in root_successors.items() 
                        if l != new_lobe
                    ]
...
def main():
    ...
    graph = setAttributeToNode(graph, ('level', 3), ('lobe', 0))
    ...
    nx.write_graphml(graph, os.path.join(target_folder, "pre-recoloring.graphml"))
