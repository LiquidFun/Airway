...
model_outer_shell_only = np.append(model_outer_shell_only, [data[np.logical_and(data == 1, model_adjacency_sum != 6)]],
                                   axis=0)
print(f"Outer shell model size: {len(model_outer_shell_only[0]):,}")
np.save(output_data_path / "bronchus_coords_outer_shell", np.array(model_outer_shell_only))
...
full_model_outer_shell = np.append(full_model_outer_shell, [data[np.logical_and(data >= 1, model_adjacency_sum != 6)]],
                                   axis=0)
print(f"Full outer shell model size: {len(full_model_outer_shell[0]):,}")
np.save(output_data_path / "full_lung_outer_shell_coords", np.array(full_model_outer_shell))
